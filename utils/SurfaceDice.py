...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm):
    ...
    kernel = np.array([[[128, 64],
                       ...
                       [[8, 4],
                        [2, 1]]])
    neighbour_code_map_gt = scipy.ndimage.filters.correlate(cropmask_gt.astype(np.uint8), kernel,
                                                            mode="constant", cval=0)
    neighbour_code_map_pred = scipy.ndimage.filters.correlate(cropmask_pred.astype(np.uint8),
    ...
    if borders_gt.any():
        distmap_gt = scipy.ndimage.morphology.distance_transform_edt(~borders_gt,
                                                                     sampling=spacing_mm)
    else:
        distmap_gt = np.Inf * np.ones(borders_gt.shape)

    if borders_pred.any():
        distmap_pred = scipy.ndimage.morphology.distance_transform_edt(~borders_pred,
                                                                       sampling=spacing_mm)
    else:
...
def compute_average_surface_distance(surface_distances):
    ...
    average_distance_gt_to_pred = np.sum(distances_gt_to_pred * surfel_areas_gt) / np.sum(
        surfel_areas_gt)
    average_distance_pred_to_gt = np.sum(distances_pred_to_gt * surfel_areas_pred) / np.sum(
        surfel_areas_pred)
...
def compute_surface_overlap_at_tolerance(surface_distances, tolerance_mm):
    ...
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
    surfel_areas_pred = surface_distances["surfel_areas_pred"]
    rel_overlap_gt = np.sum(surfel_areas_gt[distances_gt_to_pred <= tolerance_mm]) / np.sum(
        surfel_areas_gt)
    rel_overlap_pred = np.sum(surfel_areas_pred[distances_pred_to_gt <= tolerance_mm]) / np.sum(
        surfel_areas_pred)
...
print(
    "surface dice at 1mm:      {}".format(compute_surface_dice_at_tolerance(surface_distances, 1)))
...
print("expected volumetric dice: {}".format(
    2. * 100 * 100 * 100 / (100 * 100 * 100 + 102 * 100 * 100)))
...
surface_distances = compute_surface_distances(mask_gt, mask_pred, spacing_mm=(3, 2, 1))
print(
    "average surface distance: {} mm".format(compute_average_surface_distance(surface_distances)))
...
print("surface overlap at 1mm:   {}".format(
    compute_surface_overlap_at_tolerance(surface_distances, 1)))
print(
    "surface dice at 1mm:      {}".format(compute_surface_dice_at_tolerance(surface_distances, 1)))
print("volumetric dice:          {}".format(compute_dice_coefficient(mask_gt, mask_pred)))
